...
class HoneyApp extends StatefulWidget {
  @override
  HoneyCalcState createState() => HoneyCalcState();
}

class HoneyCalcState extends State<HoneyApp> {
  //variable to be used to calculate Butter calculation
  double inputHoney;
  double outputHoney;
  bool tsOtbHoney; //this variable depend on the input when
  ...
  Widget build(BuildContext context) {
    ...
    ListTile tsHoney = ListTile(
      title: const Text('Teaspoon'),
      leading: Radio(
        groupValue: tsOtbHoney,
        value: false,
        onChanged: (v) {
          setState(() {
            tsOtbHoney = v;
    ...
    Container calcBtnHoney = Container(
      child: ButtonTheme(
        minWidth: 50.0,
        height: 50.0,
        child: RaisedButton(
          child: Text("Calculate Honey"),
          color: Colors.yellow[600],
          onPressed: () {
            setState(() {
              tsOtbHoney == false
                  ? outputHoney = inputHoney * 0.141 //teaspoon calculation
                  : outputHoney = inputHoney * 0.0471; //tablespoon calculation
            });
            AlertDialog dialog = AlertDialog(
              content: tsOtbHoney == false
                  ? Text(
                      "${inputHoney.toStringAsFixed(2)} Gram : ${outputHoney.toStringAsFixed(2)} Teaspoon")
                  : Text(
                      "${inputHoney.toStringAsFixed(2)} Gram : ${outputHoney.toStringAsFixed(2)} Tablespoon"),
            );
    ...
    AppBar headerTitleHoney = AppBar(
      title: Text("Honey Measurement"),
      leading: IconButton(
    ...
    Container honeyImage = new Container(
      width: 250.0,
      height: 170.0,
      alignment: Alignment.center,
      decoration: new BoxDecoration(
        image: DecorationImage(
            image: AssetImage('pictures/honey.jpg'), fit: BoxFit.fill),
      ),
    );

    return Scaffold(
      resizeToAvoidBottomInset: false,
      appBar: headerTitleHoney,
      body: Container(
